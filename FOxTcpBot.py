...
def newinfo(uid):
    try:
        response = requests.get(f"https://projects-foxx-garena-apis.vercel.app/accinfo?uid={uid}&region=ID&api_key=CdxGfoxProjext1kmembers")
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
            f"https://mazid-gmr-like-v2.vercel.app/like?key=MAZIDxBIG-DADDY&uid={uid}&server_name=id"
        )
        
        message = ("""
[C][B][FF0000]‎━━━━━━━━━━━━
[FFFFFF]ID Tidak Valid
[FF0000]‎━━━━━━━━━━━━
""")
        ...
        if likes_api_response.status_code == 200:
            ...
            if api_json_response.get('status') != 2:
                ...
                message = f"""
[C][B][11EAFD]‎━━━━━━━━━━━━
[FFFFFF]Info Like (Before & After):

[00FF00]Like Succes Send

[FFFFFF]Nick Player : [00FF00]{player_name}  
[FFFFFF]Like Ditambahkan : [00FF00]{likes_added}  
[FFFFFF]Like Before : [00FF00]{likes_before}  
[FFFFFF]Like After : [00FF00]{likes_after}  
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Stew [FFFFFF]BOT [00FF00]Project!!
                """
            else:
                message = (
...
def get_time(uid):
    ...
    if "error" in data:
        ...
        time = f"""
[C][B][11EAFD]‎━━━━━━━━━━━━
[FFFFFF]Hari: [FFA500]{days}
[FFFFFF]Jam: [32CD32]{hours}
[FFFFFF]Menit: [1E90FF]{minutes}
[FFFFFF]Detik: [FF4500]{seconds}

[FFFFFF]Bot Stew
[00FFFF]66666666: [FF1493]@illllillilllli
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]BOT Project
        """
        return time
...
def Increase_visits(player_id):
    url = f"https://mazid-gmr-visit.vercel.app/visit?region=ID&uid={player_id}"
    res = requests.get(url)
    if res.status_code == 200:      
        data = res.json()
        uid = data["UID"]
        ...
        msg_visit = f"""
    [C][B][11EAFD]‎━━━━━━━━━━━━
    [FFFFFF]Nick Player : [00FF00]{id_name}
    [FFFFFF]ID : [00FF00]{uid}
    [FFFFFF]Jumlah Visit : [00FF00]{succvisit}
    [C][B][11EAFD]‎━━━━━━━━━━━━
    [C][B][FFB300]Stew [FFFFFF]BOT [00FF00]Project!!
    """
        return msg_visit
...
[11EAFD][B][C]
━━━━━━━━━━━━
[FFFFFF]Nickname : {id_name}
[FFFFFF]ID : {uid}
[FFFFFF]Region : {region}
‎━━━━━━━━━━━━
[808080][B][C]STEW BOT PROJECT
    """
        return msg_visit
    else:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""
[C][B][00FFFF]━━━━━━━━━━━━━━━
[FFFFFF]Eyyow Gengg!! Selamat Menggunakan BOT ini
By : Stew
[C][B][00FFFF]━━━━━━━━━━━━━━━
""", idinv
                        )
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""
[C][B][00FFFF]━━━━━━━━━━━━━━━
[FFFFFF]Recoded By : Stew

[FFFFFF]Credit : Foxx

[FFFFFF]Tampilkan Menu? Ketik :
[00FF00]/🗿help
...
[808080][b][c]Stew BOT Internal
            """,uid))
                    ...
                    try:
                        ...
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["1"]["data"]
                        clients.send(self.GenResponsMsg("[FF0000][b]❗ Gagal[/b]",uid))
                    except:
                        restart_program()
...
[11EAFD][b][c]
━━━━━━━━━━━━
Nama Player: {banned_status['player_name']}
ID : {player_id}
Status : {status}
━━━━━━━━━━━━
[808080][b][c]Stew BOT
            ...
            if "1200" in data.hex()[0:4] and b"/region/" in data:
                try:
                    ...
                    if id_match:
                        ...
                        print(f"Extracted Player ID: {player_id}")
                        if not (5 <= len(player_id) <= 15):
                            raise ValueError("Panjang Harus 15-5")
                    else:
                        raise ValueError("ID Tidak Valid")
                    json_result = get_available_room(data.hex()[10:])
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(self.GenResponsMsg("Sukses", uid))
            ...
            if "1200" in data.hex()[0:4] and b"/attack/" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(self.GenResponsMsg("Error.", uid))                   
                    b = attack_profail(player_id)
                    player_id = fix_num(player_id)
                    response_message = f"""
[11EAFD][B][C]
‎━━━━━━━━━━━━
[FFFFFF]Sukses Dikirim ke
[FFFFFF]ID Pemain : {player_id}
‎━━━━━━━━━━━━
[808080][B][C]Stew BOT
            """
                    clients.send(self.GenResponsMsg(response_message, uid))
...
[808080][B][C]Stew BOT
            ...
            if "1200" in data.hex()[0:4] and b"/ai" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]                    
                    clients.send(self.GenResponsMsg("Harap Tunggu...", uid))
                    try:
...
[00FFFF]Guild ID: [FFFFFF]{fix_num(clan_id)}
[00FFFF]Nama Guild: [FFFFFF]{clan_name}
[00FFFF]Level Guild: [FFFFFF]{clan_level}

[C][FFB300]Info Pejabat Guild:
[00FFFF]ID Leader: [FFFFFF]{clan_admin_id}
[00FFFF]Nama Lead: [FFFFFF]{clan_admin_name}
[00FFFF]EXP Lead: [FFFFFF]{clan_admin_exp}
[00FFFF]Level Leader: [FFFFFF]{clan_admin_level}
[00FFFF]BR Leader: [FFFFFF]{clan_admin_brrank}
	                        """)	                    
	                        if info_response['status'] == "ok":
	                        	...
	                        	message_info = (f"""                    
[C][FFB300]Info Dasar Leader:
[00FFFF]Region: [FFFFFF]{region}
[00FFFF]Nama: [FFFFFF]{name}
[00FFFF]Bio: [FFFFFF]{bio}
[00FFFF]Level: [FFFFFF]{level}
[00FFFF]Exp: [FFFFFF]{exp}
[00FFFF]Like: [FFFFFF]{fix_num(likes)}
[00FFFF]BR Rank: [FFFFFF]{br_rank}
{clan_info}
[FF0000]Dikirim Oleh: [FFFFFF]{sender_name}
[FF0000]ID Pengirim: [FFFFFF]{fix_num(sender_id)}
	                        """)
	                        else:
	                            message_info = (f"""
[C][B][FF0000]‎━━━━━━━━━━━━
...
[FF0000]‎━━━━━━━━━━━━
            ...
            if "1200" in data.hex()[0:4] and b"/likes/" in data:
                ...
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(self.GenResponsMsg("Gagal", uid))
                    ...
                    try:
                        ...
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["1"]["data"]
                        error_msg = f"[FF0000]Kesalahan: {e}" if "ID" in str(e) else f"[FF0000]Error: {e}"
                        clients.send(self.GenResponsMsg(error_msg, uid))
                    except:
                        restart_program()
...
[FFFFFF][B]BOT Visitor:
[FFA500]/🗿visit/104[c]145[c]933[c]49

[FFFFFF][B]Penambah Like 100:
[FF69B4]/🗿likes/104[c]145[c]933[c]49

[FFFFFF][B]ChatGPT:
[9370DB]/🗿ai

[FFFFFF][B]Mengaburkan identitas pemain:
...
"""
	                ...
	                response_mssg = f"""
[C][B][00FFFF]━━━━━━━━━━━━
[FFFFFF][B]Mengetahui server pemain:

[1E90FF]/🗿region/104[c]145[c]933[c]49

[FFFFFF]Credit : Fox
[00FF00]Recoded By : Stew
[C][B][00FFFF]━━━━━━━━━━━━
"""
	                clients.send(self.GenResponsMsg(response_mssg, uid))
    def parse_my_message(self, serialized_data):
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4018141851", password="9D2C901144CC47BC123E1CAAFE287B2BFCC07882F511D448F7D50D675E58B769")
        client_thread.start()
    except Exception as e:
